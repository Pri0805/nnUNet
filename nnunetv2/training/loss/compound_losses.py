...
###### EDITS MADE FOR TVERKSEY LOSS, also added import TverskeyLoss statement from robost_ce_loss.py ######

class CE_and_Tversky_loss(nn.Module):
    def __init__(self, tversky_kwargs, ce_kwargs, weight_ce=1, weight_tversky=1, ignore_label=None):
        ...
        :param ignore_label:
        """
        super(CE_and_Tversky_loss, self).__init__()
        if ignore_label is not None:
            ce_kwargs['ignore_index'] = ignore_label
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        tversky_loss = self.tversky(net_output, target_tversky) \
